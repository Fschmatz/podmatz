...
class MenuSection extends StatelessWidget {
  ...
  BorderRadius _radiusFor(int index) {
    ...
    return .vertical(top: index == 0 ? outer : inner, bottom: index == children.length - 1 ? outer : inner);
  }
      ...
      children: [
        if (label != null)
          Padding(
            ...
            child: Text(
              label!,
              style: tt.labelLarge?.copyWith(fontWeight: FontWeight.w600, color: cs.primary),
            ),
          ),
        for (int i = 0; i < children.length; i++) ...[if (i > 0) tileGap.gap, _positioned(children[i], i)],
      ],
    );
  }
}
